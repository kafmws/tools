...
void parse() {
	...
	cmds_list[cmds_cnt++] = &arg_list[arg_cnt];
	while(cur<p) {
		...
		while(isgraph(buf[cur])) {
			if(buf[cur] == '>') {
				...
				if(isspace(buf[cur-1]) || buf[cur-1] == 0) arg_cnt--;
				buf[cur] = 0;
	...
	arg_list[arg_cnt] = NULL;
	cmds_list[cmds_cnt+1] = (char **)(buf + p);
	cmds_list[cmds_cnt] = (char **)&cmds_list[cmds_cnt+1];

//	printf("p = %d\n",p);
//	int i = 0;
//	for(i=0;i<=cmds_cnt;i++) printf(" %p ",*cmds_list[i]);
//	printf("\n");
//	for(i=0;i<=cmds_cnt;i++) printf(" %d ",*cmds_list[i] - buf);
//	printf("\n");
}
...
void fork2exec() {
	...
	int cmdcnt = 0, rdct_cnt = 0;
	/*print the redirect bound each cmd should do*/
//	printf("%p\n%p\n",redirect_list[(rdct_cnt<<1)+1],*cmds_list[cmdcnt+1]);
	while(cmdcnt < cmds_cnt) {
		//exec cmds_list
		pid_t pid = fork();
		errat(pid<0, "fork");
		if(pid == 0) {
//			if(cmdcnt) dup2(0,pipes[0]);
//			if(cmdcnt != cmds_cnt - 1) dup2(1,pipes[1]);
...
//				printf("\n");
				...
				stderror(newfd != oldfd, "error at dup2:");
				rdct_cnt++;
			}

			/*print actually cmd*/
			int tt = 0;
			printf("%02d cmds:",cmdcnt);
			for(; cmds_list[cmdcnt][tt]; tt++)
				printf("%s ",cmds_list[cmdcnt][tt]);
			printf("\n");

			if(execvp(cmds_list[cmdcnt][0],cmds_list[cmdcnt]) == -1) {
				exit(errno);
			}
		} else {
			wait(&p_return);
