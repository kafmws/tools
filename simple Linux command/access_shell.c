...
//			printf("\np = %d\n",p);
		...
		while(exec_cnt < cmds_cnt) {
			...
			if(exec_cnt){
				pre_read = pipes[0];
				stderror(dup2(pipes[0],0), "error at dup2:");
			}
			if(exec_cnt != cmds_cnt - 1) {
				stderror(pipe(pipes),"error at pipe :");				
				stderror(dup2(pipes[1],1) != 1, "error at dup2");
			}
			exec_cnt++;
			/*print arg_list*/
//			printf("arg_cnt = %d\n",arg_cnt);
...
//			printf("\n");
			...
			if(pre_read != -1) close(pre_read);
			close(pipes[1]);
			stderror(dup2(SHELL_STDIN, 0)  != 0, "error at recover:");
			stderror(dup2(SHELL_STDOUT, 1) != 1, "error at recover:");
		}
	}
	return 0;
}
...
//	fclose(fp);
	...
	stderror((SHELL_STDIN  = dup(0)) == -1, "error at dup:");
	stderror((SHELL_STDOUT = dup(1)) == -1, "error at dup:");
}
